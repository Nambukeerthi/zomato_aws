  ...
  for i in data:
    if 'restaurants' in i:
      ...
      for j in i: 
        ...
        #print(price_range1,", ",res_name1,", ",res_id1,", ",address1,".")      
...
file_path1 = 'file1.json'        # df = pd.read_csv('s3://your-bucket-name/folder/your_file.csv', storage_options={"key": "YOUR_KEY", "secret": "YOUR_SECRET"})
...
connection_variable = 'mysql+mysqlconnector://root:root@localhost:3306/practice'
df_original.to_sql(name='zomato', con=connection_variable, if_exists='replace', index=False)

def task_1(channel):          
    db_connection_str = connection_variable
    db_connection = create_engine(db_connection_str)
    query1 = """(SELECT c.name,v.videoname FROM {channel} c JOIN videos v ON c.id=v.channelid);"""
    df_task1 = pd.read_sql(query1, con=db_connection)
    st.dataframe(df_task1, use_container_width=True)
    
    fig_bar_1 = px.bar(df_task1, x = "Brands", y = "Transaction_count", title = f" BRANDS NAME AND TRANSACTION COUNT",color_discrete_sequence= px.colors.sequential.haline, hover_name="Brands")
    st.plotly_chart(fig_bar_1, theme=None, use_container_width=True)
    
    fig_pie_1 = px.pie(df_task1, x = "Brands", y = "Transaction_count", title = f" BRANDS NAME AND TRANSACTION COUNT",color_discrete_sequence= px.colors.sequential.haline, hover_name="Brands")
    st.plotly_chart(fig_pie_1, theme=None, use_container_width=True)
...
class option:
 ...
 'Teriyaki': 143}
  ...
  option_india = ['New Delhi', 'Kolkata', 'Mumbai', 'Bangalore', 'Pune',  'Hyderabad', 'Chennai', 'Lucknow', 'Kochi',
       'Jaipur', 'Ahmedabad', 'Chandigarh', 'Goa', 'Indore', 'Nashik', 'Ludhiana', 'Guwahati', 'Amritsar', 'Kanpur',
       'Allahabad', 'Aurangabad', 'Bhopal', 'Ranchi', 'Vizag', 'Bhubaneshwar', 'Coimbatore', 'Mangalore', 'Vadodara', 'Nagpur',
       'Agra', 'Dehradun', 'Mysore', 'Puducherry', 'Surat', 'Varanasi', 'Patna']
  
  option_uae = ['Dubai',  'Sharjah', 'Abu Dhabi']
  
  option_singapore = ['Singapore']
  
  option_lanka = ['Colombo']
  
  option_turkey = ['İstanbul', 'Ankara']
  
  option_england = ['London', 'Manchester', 'Birmingham', 'Edinburgh', ' Lincoln']
  
  option_qatar = ['Doha']
  
  option_philippines = ['Makati City']
  
  option_south_africa = ['Cape Town','Sandton','Pretoria']
  
  option_brazil =['Brasília', 'São Paulo', 'Rio de Janeiro']
  
  option_new_zealand = ['Auckland', 'Wellington City']
  
  option_indonesia = ['Jakarta', ' Bandung']
  
  option_canada = [' Yorkton', ' Cochrane',' Consort',' Chatham-Kent']
  
  option_usa = [' Vernonia',' Mayfield', ' Ojo Caliente',' Vineland Station', ' Potrero', ' Princeton', ' Clatskanie', ' Fernley',
  ...
  option_australia = [' Dicky Beach',' Trentham East', ' Lakes Entrance', ' Hepburn Springs', ' Inverloch',' Weirton'
...
with st.form("my form 1"): 
        ...
        with col3:
                    ...
                    if country_name1 ==  'India':
                     city_name1 = st.selectbox(label = "CITY NAME", options = option.option_india, index =None)
                    elif country_name1 == 'United Arab Emirates':
                     city_name1 = st.selectbox(label = "CITY NAME", options = option. option_uae, index =None)
                    elif country_name1 == 'Singapore':
                     city_name1 = st.selectbox(label = "CITY NAME", options = option.option_singapore, index =None) 
                    elif country_name1 == 'Sri Lanka':
                     city_name1 = st.selectbox(label = "CITY NAME", options = option.option_lanka, index =None) 
                    elif country_name1 == 'Turkey':
                     city_name1 = st.selectbox(label = "CITY NAME", options = option.option_turkey, index =None) 
                    elif country_name1 == 'England':
                     city_name1 = st.selectbox(label = "CITY NAME", options = option.option_england, index =None) 
                    elif country_name1 == 'Qatar':
                     city_name1 = st.selectbox(label = "CITY NAME", options = option.option_qatar, index =None) 
                    elif country_name1 == 'Philippines':
                     city_name1 = st.selectbox(label = "CITY NAME", options = option.option_philippines, index =None) 
                    elif country_name1 == 'South Africa':
                     city_name1 = st.selectbox(label = "CITY NAME", options = option.option_south_africa, index =None) 
                    elif country_name1 == 'Brazil':
                     city_name1 = st.selectbox(label = "CITY NAME", options = option.option_brazil, index =None) 
                    elif country_name1 == 'New Zealand':
                     city_name1 = st.selectbox(label = "CITY NAME", options = option.option_new_zealand, index =None)
                    elif country_name1 == 'Canada':
                     city_name1 = st.selectbox(label = "CITY NAME", options = option.option_canada, index =None) 
                    elif country_name1 == 'USA':
                     city_name1 = st.selectbox(label = "CITY NAME", options = option.option_usa, index =None)
                    elif country_name1 == 'Australia':
                     city_name1 = st.selectbox(label = "CITY NAME", options = option.option_australia, index =None) 
